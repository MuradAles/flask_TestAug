...
def prepare_prompt(task="sentiment", desc=None, texts=None, label=None):
    assert task in ["sentiment", "qqp", "nli"], colored(
        "[ERROR] Unsupported test case description", "red")
...
def capture_response(response, task="sentiment"):
    ...
    if task == "sentiment":
        for choice in response.choices:
            extracted_texts.extend(
                [t.strip("-}{" + string.whitespace) for t in choice.text.split("}")])
    ...
    if task in ["qqp", "nli"]:
        for choice in response.choices:
            ...
            tup = tuple([t.strip("-}{" + string.whitespace)
                        for t in choice.text.split("\n")][:2])
            extracted_texts.append(tup)

    # keep only the string and tuple
    extracted_texts = list(filter(lambda x: isinstance(
        x, str) or isinstance(x, tuple), extracted_texts))

    # remove empty string or tuple
    extracted_texts = list(
        filter(lambda x: x != "" and x != tuple(), extracted_texts))
...
def balance_sample_by_description(df, n=1):
    ...
    ds = [k for k, v in (df.groupby(
        "description").template.nunique() == 1).to_dict().items() if v]
    ...
    if (not df.empty) and (n > 0):
        ...
        if n <= n_unique_desc:
            ts = random.sample(df.groupby("description").template.sample(
                n=1, random_state=42).tolist(), n)
        else:
            ...
            ts = df.groupby("description").template.sample(
                n=k, random_state=42).tolist()
            ts += list(random.sample(set(df.template.tolist()) -
                       set(ts), int(n - k * n_unique_desc)))
...
def generate_gpt3_test_suite(
    ...
    # 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
    seed_df = T
    ...
    for _, row in seed_df.groupby("template").agg(list).reset_index().iterrows():
        ...
        prompt = prepare_prompt(task=task, desc=desc,
                                texts=demonstrations, label=label)
        unique_texts = set()
        ...
        with tqdm(total=n_test_case_per_template) as pbar:
            while len(unique_texts) < n_test_case_per_template:
                ...
                response = query_gpt(
                    model=model, prompt=prompt, n_per_query=n_per_query)
...
def generate_new_templates(df, verbose=False):
    ...
    df = df.groupby(["capability", "description", "template",
                    "label"]).agg(list).reset_index()
        ...
        for _, row in df.iterrows():
            ...
            for gpt_text in texts:
                ...
                if isinstance(gpt_text, str):
                    ...
                    new_template = " ".join([slot_dict.get(
                        token.text, token.text) if token.pos_ in slot_tags else token.text for token in doc])
                    if capture_text_between_curly_braces(new_template) == list():
                        continue
                ...
                if isinstance(gpt_text, tuple):
                    ...
                    for gpt_text_sent in gpt_text:
                        ...
                        new_template_sent = " ".join([slot_dict.get(
                            token.text, token.text) if token.pos_ in slot_tags else token.text for token in doc])
                        if capture_text_between_curly_braces(new_template_sent) == list():
                            continue
    ...
    df = pd.DataFrame(records).drop_duplicates(
        subset="new_template").reset_index(drop=True)
    return df
...
def prepare_testing(df_dict):
    ...
    for name in df_dict.keys():
        df_dict[name] = df_dict[name].sample(
            n=n, random_state=42).assign(source=name)
    ...
    for df in df_dict.values():
        ...
        sample_size = int(
            n_unique_template / 2) if n_unique_template % 2 == 0 else int((n_unique_template + 1) / 2)
...
def test_model(model_name, model_type, task, num_labels, patch_df, test_df):
    ...
    patch_df = patch_df[patch_df.text.apply(
